...
// Helpers Imports
import { requestDeleteProduct, requestGetProducts } from '@helpers/request';
...
const Products = () => {
  ...
  const canEdit = hasAllPermissions('products.edit', admin.permissions);
  const canDelete = hasAllPermissions('products.delete', admin.permissions);

  const { t, i18n } = useTranslation();
  const textT: any = useMemo(() => t('products:text', { returnObjects: true, default: {} }), [t]);
  ...
  const handleFetchProducts = async () => {
    ...
    const result = await requestGetProducts(params, i18n.language);
  ...
  const handleDelete = async () => {
    ...
    const result = await requestDeleteProduct(deleteState.id || 0, i18n.language);
  ...
  const columns: GridColDef[] = [
    {
      field: 'code',
      headerName: textT?.table?.code?.title,
      flex: 1,
      minWidth: 200,
      renderCell: (params) =>
        canEdit ? (
    ...
    {
      field: 'cabys',
      headerName: textT?.table?.cabys?.title,
      flex: 1,
      minWidth: 200
    },
    {
