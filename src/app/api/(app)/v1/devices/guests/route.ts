...
/**
 * @swagger
 * /api/v1/devices/guests:
 *  post:
 *    description: Register guest device
 *    operationId: GuestDevice
 *    tags:
 *      - Devices
 *    security:
 *      - ApiKey: []
 ...
 *              platform:
 *                type: string
 *                description: Guest device platform
 *                enum: [IOS, ANDROID]
 *              token:
 *                type: string
 ...
 *      allOf:
 *        - 200:
 *            description: Guest device registration success
 *            content:
 *              application/json:
 *                schema:
 *                  $ref: '#/definitions/responses/devices/guests/post'
...
export const POST = appProxy(false, async (req) => {
  ...
  try {
    ...
    if (data.token) {
      unsubscribeDeviceFromTopic(data.token, 'REGISTERED_USERS');
      subscribeDeviceToTopic(data.token, 'GUEST_USERS');
    }
