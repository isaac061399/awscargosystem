...
export const POST = async (req: NextRequest) => {
  ...
  const textT: any = t('api:auth/forgot-password', { returnObjects: true, default: {} });
  ...
  try {
    await withTransaction(async (tx) => {
      ...
      await tx.userLog.create({
        data: { user_id: user.id, action: 'forgot-password' }
      });
      ...
      await sendEmail({
        ...
        replaceData: {
          name: user.name,
          resetLink: `${process.env.NEXTAUTH_URL}/auth/reset-password/${token}/${user.id}`,
