...
// Helper Imports
import { requestReset2FA } from '@helpers/request';

const Reset2FA = ({ token, userId }: { token: string; userId: string }) => {
  const { t, i18n } = useTranslation('reset-2fa');
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        const result: any = await requestReset2FA(
