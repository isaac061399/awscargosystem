...
// Helpers Imports
import { requestDeleteCategory, requestGetCategories } from '@helpers/request';
...
const Categories = () => {
  ...
  const canCreate = hasAllPermissions('categories.create', admin.permissions);
  const canEdit = hasAllPermissions('categories.edit', admin.permissions);
  const canDelete = hasAllPermissions('categories.delete', admin.permissions);

  const { t, i18n } = useTranslation();
  const textT: any = useMemo(() => t('categories:text', { returnObjects: true, default: {} }), [t]);
  ...
  const handleFetchCategories = async () => {
    ...
    const result = await requestGetCategories(params, i18n.language);
  ...
  const handleDelete = async () => {
    ...
    const result = await requestDeleteCategory(deleteState.id || 0, i18n.language);
  ...
  const columns: GridColDef[] = [
    {
      ...
      renderCell: (params) =>
        canEdit ? (
          <Link
            href={`/categories/edit/${params.row.id}`}
            ...
            {canCreate && (
              <Tooltip title={textT?.table?.actions?.duplicateLabel} placement="top">
                <IconButton
                  ...
                  size="small"
                  LinkComponent={Link}
                  href={`/categories/new?ref=${params.row.id}`}>
        ...
        <Grid size={{ xs: 12 }}>
          <div className="flex gap-2">
            <FilterSearch
              ...
              onSearch={handleFetchCategories}
            />
            <LocaleFilter
              value={localeState}
              onChange={(e) => {
