...
// Helpers Imports
import { requestDeleteContent, requestGetContents } from '@helpers/request';
...
const Contents = () => {
  ...
  const canCreate = hasAllPermissions('contents.create', admin.permissions);
  const canEdit = hasAllPermissions('contents.edit', admin.permissions);
  const canDelete = hasAllPermissions('contents.delete', admin.permissions);

  const { t, i18n } = useTranslation();
  const textT: any = useMemo(() => t('contents:text', { returnObjects: true, default: {} }), [t]);
  ...
  const handleFetchContents = async () => {
    ...
    const result = await requestGetContents(params, i18n.language);
  ...
  const handleDelete = async () => {
    ...
    const result = await requestDeleteContent(deleteState.id || 0, i18n.language);
  ...
  const columns: GridColDef[] = [
    {
      ...
      renderCell: (params) =>
        canEdit ? (
          <Link
            href={`/contents/edit/${params.row.id}`}
    ...
    {
      field: 'category',
      headerName: textT?.table?.category?.title,
      flex: 1,
      minWidth: 200,
    ...
    {
      field: 'page',
      headerName: textT?.table?.page?.title,
      flex: 1,
      minWidth: 200,
      renderCell: (params: any) => `${params.row.page?.name || ''}`
    },
    {
      field: 'locale',
            ...
            {canCreate && (
              <Tooltip title={textT?.table?.actions?.duplicateLabel} placement="top">
                <IconButton
                  ...
                  size="small"
                  LinkComponent={Link}
                  href={`/contents/new?ref=${params.row.id}`}>
