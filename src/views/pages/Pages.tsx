...
// Helpers Imports
import { requestDeletePage, requestGetPages } from '@helpers/request';
...
const Pages = () => {
  ...
  const canCreate = hasAllPermissions('pages.create', admin.permissions);
  const canEdit = hasAllPermissions('pages.edit', admin.permissions);
  const canDelete = hasAllPermissions('pages.delete', admin.permissions);

  const { t, i18n } = useTranslation();
  const textT: any = useMemo(() => t('pages:text', { returnObjects: true, default: {} }), [t]);
  ...
  const handleFetchPages = async () => {
    ...
    const result = await requestGetPages(params, i18n.language);
  ...
  const handleDelete = async () => {
    ...
    const result = await requestDeletePage(deleteState.id || 0, i18n.language);
            ...
            {canCreate && (
              <Tooltip title={textT?.table?.actions?.duplicateLabel} placement="top">
                <IconButton
                  ...
                  size="small"
                  LinkComponent={Link}
                  href={`/pages/new?ref=${params.row.id}`}>
