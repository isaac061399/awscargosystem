...
// Helpers Imports
import { requestDeleteRole, requestGetRoles } from '@helpers/request';
...
const Roles = () => {
  ...
  const canEdit = hasAllPermissions('roles.edit', admin.permissions);
  const canDelete = hasAllPermissions('roles.delete', admin.permissions);

  const { t, i18n } = useTranslation();
  const textT: any = useMemo(() => t('roles:text', { returnObjects: true, default: {} }), [t]);
  ...
  const handleFetchRoles = async () => {
    ...
    const result = await requestGetRoles(params, i18n.language);
  ...
  const handleDelete = async () => {
    ...
    const result = await requestDeleteRole(deleteState.id || 0, i18n.language);
